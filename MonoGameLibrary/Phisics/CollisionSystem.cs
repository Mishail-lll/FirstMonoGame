...
public class CollisionSystem
{
    ...
    public bool NarrowPhase(Collider A, Collider B)
    {
    ...
    public void ProcessCollisions()
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            for (int j = 0; j < n; j++)
            {
                ...
                if (!_hasCallback[A.Layer, B.Layer] && !_hasEnterHandler[A.Layer, B.Layer] && !_hasExitHandler[A.Layer, B.Layer] && !_layerHandlerNonFalseOnLayer[A.Layer] && !_layerHandlerNonFalseOnLayer[B.Layer]) continue;
